...
class DurationPredictor(torch.nn.Module):
    ...
    def _forward(self, xs, x_masks=None, spembs_=None, is_inference=False):
        xs = xs.transpose(1, -1)  # (B, idim, Tmax)
        if self.hparams.is_spk_layer_norm:
            for i in range(len(self.conv)):
                xs = self.conv[i](xs)  # (B, C, Tmax)
                xs = self.norm[i](xs, spembs_)
                xs = self.dropout[i](xs)
        else:
            for f in self.conv:
                xs = f(xs)  # (B, C, Tmax)
    ...
    def forward(self, xs, x_masks=None, spembs_=None):
        ...
        """
        return self._forward(xs, x_masks, spembs_=spembs_, is_inference=False)

    def inference(self, xs, x_masks=None, spembs_=None):
        """Inference duration.
        ...
        """
        return self._forward(xs, x_masks, spembs_=spembs_, is_inference=True)
