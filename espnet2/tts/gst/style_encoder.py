...
class StyleEncoder(torch.nn.Module):
    ...
    def forward(self, speech: torch.Tensor, spembs_: torch.Tensor = None) -> torch.Tensor:
        ...
        """
        ref_embs = self.ref_enc(speech, spembs_=spembs_ if self.hparams.style_vector_type == 'mha' else None)
...
class ReferenceEncoder(torch.nn.Module):
    ...
    def __init__(
        ...
        if self.hparams.style_vector_type == 'mha':
            self.mha_linear = torch.nn.Linear(gru_in_units, gru_units)
            self.mha_vector = MultiHeadedAttention(
            q_dim=gru_units,
            k_dim=gru_units,
            v_dim=gru_units,
            n_head=4,
            n_feat=gru_units,
            dropout_rate=0.0,)
            self.mha_norm = LayerNorm(gru_units,hparams=hparams)
    ...
    def forward(self, speech: torch.Tensor, spembs_: torch.Tensor = None) -> torch.Tensor:
        ...
        if self.hparams.style_vector_type == 'mha':
            ...
            ref_embs = self.mha_linear(hs) # (B, Lmax', gru_units)
            ref_embs = self.mha_vector(ref_embs, ref_embs, ref_embs, None)# (B, Lmax', gru_units)
            ref_embs = self.mha_norm(ref_embs, spembs_)# (B, Lmax', gru_units)
            ref_embs = ref_embs.mean(dim=1)# (B, gru_units)
        return ref_embs
...
class ChooseStyleTokenLayer(torch.nn.Module):
    ...
    def forward(self, ref_embs: torch.Tensor, style_tokens: torch.Tensor) -> torch.Tensor:
        ...
        # NOTE(kan-bayashi): Shoule we apply Tanh?
        # ref_embs = ref_embs.unsqueeze(1)  # (batch_size, 1 ,ref_embed_dim)
        style_embs = self.choose_mha(ref_embs, style_tokens, style_tokens, None)
