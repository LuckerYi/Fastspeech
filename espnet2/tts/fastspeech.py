...
class FastSpeech(AbsTTS):
        ...
        if self.use_gst:
            if self.hparams.is_partial_refine and self.hparams.is_refine_style:
                ...
                for i in range(len(utt_mels)):
                    style_embs.append(self.gst(to_gpu(utt_mels[i]), spembs_=spembs_))#(1, gst_token_dim)
                ...
                style_embs = self.gst.choosestl(hs, style_tokens)#(B, Tx, gst_token_dim)
            else:
                style_embs = self.gst(ys, spembs_=spembs_)
        # integrate with GST
            if self.hparams.style_embed_integration_type == "concat":
                hs = self.gst_projection(torch.cat([hs,style_embs.unsqueeze(1).expand(-1, hs.size(1), -1)],dim=-1))
    ...
    def forward(
        ...
        before_outs, after_outs, d_outs = self._forward(
            xs, ilens, ys, olens, ds, spk_ids=spk_ids, style_ids=style_ids, utt_mels=utt_mels, is_inference=False
        )
    ...
    def inference(
        ...
        if use_teacher_forcing:
            ...
            # inference
            _, outs, _ = self._forward(
                xs, ilens, ys, spk_ids=spk_ids, is_inference=True, alpha=alpha, utt_mels=utt_mels
            )  # (1, L, odim)
    ...
    def _parse_batch(batch, hparams, utt_mels=None):
        ...
        return (text_padded, input_lengths, mel_padded, output_lengths, duration_padded,
                duration_lengths, speaker_ids, style_ids, utt_mels)